    ...
    <div class="col-md-10 mx-auto">
        <div class="card">
            <div class="card-header py-2">
                <h6 class="card-title mb-0">Update School Details</h6>
            </div>
            <form action="{{ route('schools.update', $school->id) }}" method="POST">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="registration_number" class="form-control @error('registration_number') is-invalid @enderror" value="{{ old('registration_number', $school->registration_number) }}" required placeholder="e.g. S.0123/456">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="category" class="form-select @error('category') is-invalid @enderror" required>
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->name }}" {{ old('category', $school->category) == $cat->name ? 'selected' : '' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                            @error('category')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                                ...
                                @foreach($types as $type)
                                    <option value="{{ $type->name }}" {{ old('type', $school->type) == $type->name ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="region" id="regionSelect" class="form-select @error('region') is-invalid @enderror" required>
                                ...
                                @foreach($regions as $region)
                                    <option value="{{ $region->name }}" data-id="{{ $region->id }}" {{ old('region', $school->region) == $region->name ? 'selected' : '' }}>{{ $region->name }}</option>
                                @endforeach
                            </select>
                            @error('region')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="district" id="districtSelect" class="form-select @error('district') is-invalid @enderror" required>
                                ...
                                @foreach($districts as $district)
                                    <option value="{{ $district->name }}" {{ old('district', $school->district) == $district->name ? 'selected' : '' }}>{{ $district->name }}</option>
                                @endforeach
                            </select>
                            @error('district')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="ward" class="form-control" value="{{ old('ward', $school->ward) }}">
                        </div>
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="headmaster_name" class="form-control" value="{{ old('headmaster_name', $school->headmaster_name) }}">
                        </div>
                ...
                <div class="card-footer d-flex justify-content-between">
                    ...
                    <button type="submit" class="btn btn-primary px-4">Update School</button>
                </div>
            </form>
        </div>
    </div>
