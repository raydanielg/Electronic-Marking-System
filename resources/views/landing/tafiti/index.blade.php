    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .item-cover::after {
            ...
            position: absolute;
            inset: 0;
            background: linear-gradient(180deg, rgba(2,6,23,0.12) 0%, rgba(2,6,23,0.60) 100%);
        }
...
<body>
    ...
    <section class="page-hero">
        ...
        <div class="container">
            <div class="page-hero-inner" data-aos="fade-up">
                <div class="page-kicker"><i class="fas fa-flask"></i> Tafiti</div>
                <h1 class="page-title">Research & Insights</h1>
                <p class="page-desc">
                    Briefs, studies, and practical insights to strengthen exam processing, analytics, and education governance.
                </p>
                ...
                <div class="row g-3 align-items-center">
                    <div class="col-lg-7">
                        <div class="input-group">
                            ...
                            <input id="tafitiSearch" type="text" class="form-control" placeholder="Search tafiti (e.g., analytics, security, data quality)..." style="border-radius: 0 9999px 9999px 0; font-weight: 800;" />
                        </div>
                    </div>
                    <div class="col-lg-5 d-flex justify-content-lg-end">
                        <span style="font-weight: 900; color:#475569;">Showing <span id="tafitiCount">0</span> items</span>
                    </div>
                </div>
            </div>

            @if($items->count())
                <div class="row g-4 mt-2" id="tafitiGrid">
                    @foreach($items as $it)
                        <div class="col-lg-4 tafiti-item" data-title="{{ strtolower($it->title) }}" data-excerpt="{{ strtolower($it->excerpt ?? '') }}">
                            <a class="item-card" href="{{ route('landing.tafiti.show', $it->slug) }}" data-aos="fade-up" data-aos-delay="100">
                                <div class="item-cover">
                                    <img src="{{ $it->cover_image ? asset($it->cover_image) : asset('hero/students-paying-attention-class.jpg') }}" alt="{{ $it->title }}">
                                    <div class="item-meta">
                                        <span class="meta-pill"><i class="fas fa-calendar"></i> {{ optional($it->published_at)->format('d M Y') ?? '—' }}</span>
                                        <span class="meta-pill"><i class="fas fa-building"></i> {{ $it->organization ?? 'EMaS' }}</span>
                                    </div>
                                </div>
                                <div class="item-body">
                                    <div class="item-title">{{ $it->title }}</div>
                                    <p class="item-excerpt">{{ $it->excerpt }}</p>
                                    <div class="item-cta">Open brief <i class="fas fa-arrow-right"></i></div>
                                </div>
                            </a>
                        </div>
                    @endforeach
                </div>
                <div class="mt-4" id="tafitiEmpty" style="display:none;">
                    <div class="empty-state">No items match your search.</div>
                </div>
            @else
                <div class="mt-4" data-aos="fade-up">
                    <div class="empty-state">No tafiti items yet. Check back soon for new research briefs.</div>
                </div>
            @endif
        </div>
    </section>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const search = document.getElementById('tafitiSearch');
            const items = Array.from(document.querySelectorAll('.tafiti-item'));
            const empty = document.getElementById('tafitiEmpty');
            const count = document.getElementById('tafitiCount');
